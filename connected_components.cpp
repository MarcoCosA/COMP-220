...
class ConnectedComponent{
  public:
    ...
    void dfsCC(Graph *g, int vid){ // Depth first search method
      ...
      Node * vList = g->getAdjVertices(vid);
      compId[vid]=componentCount;              

      for(Node * tv = vList; tv!=nullptr; tv=tv->next){ 
          if(!wasVisited[tv->id]){
            dfsCC(g, tv->id);
          }
      }
...
//TODO
    if(getComponentId(va) == getComponentId(vb))
        return true;
    return false;
    }
...
int main() {
 ...
 // interactive mode below
  ...
  while( true ){ // loop forever
    ...
    x = getIntegerInput(); // get an integer from input until success
    z = getIntegerInput(); // get an integer from input until success
    if (x >= 0 && z >= 0){
      if (cc.isConnectedComponent( x, z ))
         cout << "Yes, " << x << " and " << z << " are in the same component " << endl;
      else
         cout << "No, " << x << " and " << z << " are NOT the same component " << endl;
    }
    else
